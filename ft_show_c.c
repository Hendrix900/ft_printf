...
/*   Updated: 2020/07/15 20:01:01 by ccastill         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_show_c(const char *s, t_list_printf *next)
{
	...
	if (next->c == '\0' && next->type != '%')
	{
		...
		next->br++; // Añadir un contador de posiciones a ft_putchar
	}
	else if (next->type == '%') 
		new[0] = s[next->len];
	else
	...
	if (next->flags >= 1 && next->punt > 1)
		...
		next->type == 'c' && next->c == '\0' ? ft_putchar_fd(0,1) : 
		ft_putstr_fd(new, 1, next);	free(new);
	new = NULL;
}
